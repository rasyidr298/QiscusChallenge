...
class FictionBookLibrary {
    ...
    List<Book> bookContains = new ArrayList<>();
    Member member;

    public void addCollection(Book book, Integer count){
    ...
    public List<Book> listBooksAuthoredBy(String author){
        for (int i = 0; i<bookAvailable.size(); i++){
            if (author.contains(bookAvailable.get(i).getAuthor())){
                bookContains.add(bookAvailable.get(i));
            }
        }
        return bookContains;
    }
...
class Member {
    private String name;
    private List<Book> borrowBook = new ArrayList<>();
